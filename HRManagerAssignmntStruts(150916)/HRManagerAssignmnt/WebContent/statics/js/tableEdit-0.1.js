window.onload = function() {
	$.fn.tableEdit = function(settings, callback, activeMasks) {
		...
		var defaults = {
			...
			columnsTr : null, // index td editable, if null all td editables
			// Ex.: "1,2,3,4,5"
			classBtEdit : "btEdit", // class of button for click event

			enableDblClick : true,

			textBtSave : "<span class='action' action='Save'><img src='statics/images/save.ico' /></span>", // text
			// button
			// save
			textBtEdit : "<span class='action' action='Edit'><img src='statics/images/edit.png' /> </span>", // text
			// button
			// edit
		...
		return this.each(function() {
			...
			var tdsIndex = (settings.columnsTr != null) ? settings.columnsTr
					.split(",") : null;
			// get all rows
			...
			for (k = 0; k < trsTable.length; k++) {
				...
				$.each(tdsLine, function(index, td) {
					...
					if ($(td).find("." + settings.classBtEdit).length == 0) {
						/*
						 * insert class edition for all columns or columns in
						 * tdsIndex
						 */
						if (tdsIndex == null) {
			...
			function mountNewInput(cell) {
				...
				if (attrsString != null) {
					...
					for (n = 0; n < attrsArray.length; n++) {
						...
						$(element).attr($.trim(currentObj[0]),
								$.trim(currentObj[1]));
					}
				} else {
				...
				element
						.setAttribute("style", "width:" + $(cell).width()
								+ "px");
			...
			function getCallback() {
				settings.callback(settings.callBackObject);// how can I send
				// var a,b here
			}
			...
			function changeBt(bt) {
				...
				$(bt).attr("class", "").addClass(
						((hasClass) ? "btSave" : settings.classBtEdit));

				$(bt)
						.html(
								((hasClass) ? settings.textBtSave
										: settings.textBtEdit));
				$(bt)
						.val(
								((hasClass) ? settings.textBtSave
			...
			function dblClickEvent() {
				...
				if (settings.enableDblClick) {
					elementBt = $(this).parent().find(
							"." + settings.classBtEdit);
					changeBt(elementBt);
					editTd($(this));
				}
			}
